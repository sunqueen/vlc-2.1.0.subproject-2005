                ...
                int protocol)
{
    ...
    setsockopt (fd, SOL_SOCKET, SO_REUSEADDR, (char *)&offset/*(int){ 1 }*/, sizeof (int));			// sunqueen modify
     ...
     */
    if (family == AF_INET6)
        setsockopt (fd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&offset/*(int){ 1 }*/, sizeof (int));			// sunqueen modify
#endif
...
// sunqueen modify start
	{
		offset = PROTECTION_LEVEL_UNRESTRICTED;
        setsockopt (fd, IPPROTO_IPV6, IPV6_PROTECTION_LEVEL,
                    (char *)&offset/*(int){ PROTECTION_LEVEL_UNRESTRICTED }*/, sizeof (int));
    }
// sunqueen modify end
#endif
 ...
 *****************************************************************************/
ssize_t
net_Read (vlc_object_t *__restrict p_this, int fd, const v_socket_t *vs,
          void *__restrict p_buf, size_t i_buflen, bool waitall)			// sunqueen modify
{
    struct pollfd ufd[2];
    ...
    do
    {
        ...
        {
#ifdef _WIN32
            n = recv (fd, (char *)p_buf, i_buflen, 0);			// sunqueen modify

#else
            n = read (fd, (char *)p_buf, i_buflen);			// sunqueen modify
#endif
        }
...
ssize_t net_Write( vlc_object_t *p_this, int fd, const v_socket_t *p_vs,
                   const void *__restrict p_data, size_t i_data )			// sunqueen modify
{
    size_t i_total = 0;
// sunqueen modify start
//    struct pollfd ufd[2] = {
...
#ifdef _WIN32
            val = send (fd, (const char *)p_data, i_data, 0);			// sunqueen modify
#else
...
char *net_Gets(vlc_object_t *obj, int fd, const v_socket_t *vs)
{
    ...
    for (;;)
    {
        if (buflen == bufsize)
        {
            ...
            char *newbuf = (char *)realloc(buf, bufsize + 1024);			// sunqueen modify
